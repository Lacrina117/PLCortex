...
const FeatureCard: React.FC<{ title: string; description: string; icon: React.ReactNode; onClick: () => void; accentColor: string }> = ({ title, description, icon, onClick, accentColor }) => (
    <button
        onClick={onClick}
        className="group text-left w-full h-full bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/50 hover:shadow-xl hover:border-transparent transition-all duration-300"
    >
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 duration-300 bg-${accentColor}-100 dark:bg-${accentColor}-900/30 text-${accentColor}-600 dark:text-${accentColor}-400`}>
            {icon}
        </div>
        <h3 className="text-lg font-black text-gray-900 dark:text-gray-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{title}</h3>
        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400 leading-relaxed font-medium">{description}</p>
    </button>
);
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-12">
            <header className="animate-fade-in text-center md:text-left">
                <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight sm:text-5xl mb-4">
                    {t('dashboard.title')}
                </h1>
                <p className="max-w-2xl text-xl text-gray-500 dark:text-gray-400 font-medium">
            ...
            {visibleRecentActivity.length > 0 && (
                <section className="animate-fade-in-up">
                    <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-6">{t('dashboard.recentActivity')}</h2>
            ...
            <section className="animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
                <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-6">{t('dashboard.toolsSection')}</h2>
